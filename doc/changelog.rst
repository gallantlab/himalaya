...
- FIX device mismatch when ``sample_weight`` is passed as a numpy array to
  kernel ridge estimators with a GPU/MPS backend.
- MNT install CPU-only PyTorch in CI to avoid unnecessary CUDA downloads.
